...
resource "aws_security_group" "wp_dev_sg" {
  name        = "wp_dev_sg"
  description = "used to access teh dev from my IP"
  vpc_id      = "${aws_vpc.wp_vpc.id}"
...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile papa &&  ansible-playbook -i aws_hosts wordpress.yml"
  }
}
